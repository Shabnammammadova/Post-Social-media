                ...
                <ul className="pt-5">
                    {Navigate.map((menu, index) => (
                        <li className={`flex items-center gap-x-4 text-blue-900 text-md font-bold cursor-pointer p-2 hover:bg-light-white rounded-md`} key={index}>
                            ...
                            <a href={menu.path} className={`${!open && 'hidden'} origin-left duration-200`}>{menu.title}</a>
                        </li>
                    ))}
                    {user && (
                        <button className="text-gray-800" disabled={loading} onClick={() => dispatch(logoutAsync())}>
                            Logout
                        </button>
                    )}
                </ul>
